    ...
    if (strcmp($result, "VERIFIED") == 0) {          // VALID PAYMENT!
        ...
        if ($data->payment_status != "Completed" and $data->payment_status != "Pending") {
			...
			if ($data->payment_status == "Refunded") {
				...
				$subject = get_string('paypalrequestrefundsubject', 'enrol_paypalrequest');
				$emailparams = new stdClass();
				$emailparams->participantname   = $data->option_selection1;
				$emailparams->coursefullname    = $data->item_name;
				$emailparams->courseurl         = $CFG->wwwroot.'/course/view.php?id='.$data->courseid;
				$emailparams->participantemail  = $data->payer_email;
				
				$msgbody = get_string('paypalrequestrefundbody', 'enrol_paypalrequest', $emailparams);
				$bodyhtml = '<head>';
				$bodyhtml .= '</head>'.
					"\n<body id=\"enrolpaypalrequest_refundnotify\">\n<div id=\"page\">\n".
					$msgbody.
					"\n</div>\n</body>";

				if (!(email_to_user($admin, $site->fullname, $subject, $msgbody, $bodyhtml))) {
					$errors[] = 'Failed to send email to user: '.fullname($admin);
				}
			}
		...
		// Integration with enrol_request - Change status of users that were successfully enrolled
